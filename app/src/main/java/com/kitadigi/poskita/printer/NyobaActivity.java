...
public class NyobaActivity extends BaseActivity implements OnClickListener{
/******************************************************************************************************/
	// Debugging
...
/******************************************************************************************************/
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//====================================================================
		printerHelper=new PrinterHelper(NyobaActivity.this);
...
/****************************************************************************************************/
	...
	private void Print_Ex(){
		...
		if (is58mm) {
			...
			if (is58mm) {
				...
				try {
					byte[] qrcode = PrinterCommand.getBarCommand("2018123100001", 0, 3, 6);//
					Command.ESC_Align[2] = 0x01;
					SendDataByte(Command.ESC_Align);
					SendDataByte(qrcode);
					...
					Command.GS_ExclamationMark[2] = 0x11;
					SendDataByte(Command.GS_ExclamationMark);
					SendDataByte("TIKET PARKIR\n".getBytes("GBK"));
					Command.ESC_Align[2] = 0x00;
					SendDataByte(Command.ESC_Align);
					Command.GS_ExclamationMark[2] = 0x00;
					SendDataByte(Command.GS_ExclamationMark);
					SendDataByte("Melayani reservasi tiket kereta api &amp;\npesawat, rental mobil, antar jemput\nstasiun / bandara, city tour, parkir inap\nmobil\n================================\n".getBytes("GBK"));
					SendDataByte("Office : Jl. Siliwangi Gg. Kramat 5, Cirebon\nPhone : 0231 - 233777 - 082321112448\nWhatsapp : 082321112448\n(*) Ket : Kunci ditinggal &amp; Cuci Full\n".getBytes("GBK"));
					Command.ESC_Align[2] = 0x01;
					SendDataByte(Command.ESC_Align);
					Command.GS_ExclamationMark[2] = 0x11;
					SendDataByte(Command.GS_ExclamationMark);
	...
	private void printBarCode() {
		...
		new AlertDialog.Builder(this).setTitle(getText(R.string.btn_prtcode))
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if(which == 0)
				{
					if(str.length() == 11 || str.length() == 12)
					{
						...
						Toast.makeText(NyobaActivity.this, getText(R.string.msg_error), Toast.LENGTH_SHORT).show();
						return;
					}
				}
				else if(which == 1)
				...
				else if(which == 9)
				{
					if(str.length() == 0)
					{
						Toast.makeText(NyobaActivity.this, getText(R.string.empty1), Toast.LENGTH_SHORT).show();
						return;
					}
					else
					{
	...
	private void CommandTest(){
		...
		String lang = getString(R.string.strLang);
		if((lang.compareTo("cn")) == 0){
		new AlertDialog.Builder(NyobaActivity.this).setTitle(getText(R.string.chosecommand))
